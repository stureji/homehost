...
test('ShoppingList: toJson()', t => {
  ...
  t.is(json.list[0].checked, false, 'expecting false check marks');
  t.is(json.list[1].checked, false, 'expecting false check marks');
  t.is(json.list[0].grocery, 'Spaghetti', 'expecting Spaghetti name');
  t.is(json.list[1].grocery, 'Tomatoes', 'expecting Tomatoes name');
  list.add(parmesan);
  list.remove(tomatoes);
  list.list[1].toggle();
  const mutedJson = list.toJson();
  t.is(mutedJson.list[0].checked, false, 'expecting false check mark');
  t.is(mutedJson.list[1].checked, true, 'expecting ture check mark');
  t.is(mutedJson.list[0].grocery, 'Spaghetti', 'expecting Spaghetti name');
  t.is(mutedJson.list[1].grocery, 'Parmesan Cheese', 'expecting Parmesan Cheese name');
});
